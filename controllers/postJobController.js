...
const getAllJobs = async (req, res) => {
    try {
        ...
        const findAllJobs = await postJobsModel.findAll({ where: { company_id: company_id } });
        if (findAllJobs) {
...
const getAllAvailableJobs = async (req, res) => {
    try {
        const findAllJobs = await postJobsModel.findAll({attributes:{exclude:["id","company_id","deletedAt"]}});
        if (findAllJobs) {
            res.status(200).json(findAllJobs)
        }
    } catch (error) {
