...
const registerCompanyController = async(req,res)=>{
  try {
    ...
    if (!companyName || !companyEmail || !password || !companyPhoneNumber) {console.log("check required fields");return;
    }
    const token = req.token;//hash the password
    ...
    if(findCompany){res.status(403).json("Company already exist. Please login!");
      return;
    }

    CompaniesModel.create(newCompany).then(() => {
